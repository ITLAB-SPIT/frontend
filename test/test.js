...
suite(function (env) {
  describe("Login and visit script", function () {
    ...
    it("Login and Check all the navbar links, at the end logout", async function () {
      ...
      let excersieButton = await driver.wait(
        until.elementLocated(By.partialLinkText("Exercises")),
        5000
      );
      excersieButton.click();

      let reviewButton = await driver.wait(
        until.elementLocated(By.partialLinkText("Review")),
        5000
      );
      reviewButton.click();

      let newsButton = await driver.wait(
        until.elementLocated(By.partialLinkText("News")),
        5000
      );
      newsButton.click();

      let workoutButton = await driver.wait(
        until.elementLocated(By.partialLinkText("Workouts")),
        5000
      );
      workoutButton.click();
    ...
    it("BMI visit and calculation", async function () {
      await driver.get(url);
      let bmiButton = await driver.wait(
        until.elementLocated(By.partialLinkText("BMI Calculator")),
        5000
      );
      bmiButton.click();

      let heightTextBox = await driver.wait(
        until.elementLocated(By.name(`bmi_height`)),
        5000
      );
      let weightTextBox = await driver.wait(
        until.elementLocated(By.name(`bmi_weight`)),
        5000
      );
      let ageTextBox = await driver.wait(
        until.elementLocated(By.name(`bmi_age`)),
        5000
      );
      let submitButton = await driver.wait(
        until.elementLocated(By.name(`bmi_submit`)),
        5000
      );
